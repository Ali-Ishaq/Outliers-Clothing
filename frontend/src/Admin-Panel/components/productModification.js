    ...
    <div id='productModificationComponent'>
        <div className="productIdModificationForm">
            ...
            <div className="actionBtns">
                <button className='updateBtns' onClick={()=>{if(idRef.current.value.length>0){handleClick('delete')}}}>Delete Product</button>
                <button onClick={()=>{if(idRef.current.value.length>0){handleClick('update')  }}} className='updateBtns'>Edit Product</button>
            </div>
        </div>
    </div>
  )
